...
export const canEditInitiative = (
  ...
  if (record.record_type === "CARD" && (record.is_locked ?? isPeriodLocked(record.year, record.quarter))) return role.canEditArchive;
  return true;
};
...
export const canDeleteInitiative = (
  ...
  if (record.record_type === "CARD" && (record.is_locked ?? isPeriodLocked(record.year, record.quarter))) return false;
  return true;
};
