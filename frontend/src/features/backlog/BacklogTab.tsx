...
export const BacklogTab = () => {
  ...
  const toggleQuarter = (master: Initiative, quarter: Quarter) => {
    ...
    const previousCard = cardsFor(master.id).sort((a, b) =>
    ...
    const result =
      activeTab === "PROJECTS"
        ? addProject(card as Project)
        : addTask(card as OperationalTask);
  ...
  const columnCount =
    3 + visibleQuarters.length + (isSelectingForExtension ? 1 : 0);
    ...
    <div className="backlog-page space-y-9 pt-2.5 text-slate-900">
      <section className="rounded-[24px] border border-slate-200 bg-white px-5 py-4 shadow-[0_2px_5px_rgba(15,23,42,0.10)] sm:px-7 sm:py-5 lg:px-8">
        <div className="grid gap-4 2xl:grid-cols-[auto_minmax(0,1fr)] 2xl:items-center">
          <h1 className="min-w-fit text-[30px] font-extrabold tracking-[-0.02em] text-slate-900 sm:text-[36px]">
            Беклог
          </h1>

          <div
            className={`grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-[minmax(150px,0.8fr)_minmax(260px,1.7fr)] 2xl:ml-10 ${isSelectingForExtension ? "2xl:grid-cols-[160px_minmax(270px,1fr)_minmax(440px,1.5fr)]" : "2xl:grid-cols-[160px_minmax(270px,1fr)_minmax(300px,1.12fr)_174px]"}`}
          >
            <select
              aria-label="Рік беклогу"
              value={selectedYear}
              onChange={(event) => changeYear(Number(event.target.value))}
              className="h-[52px] min-w-0 rounded-2xl border border-slate-300 bg-white px-4 text-[16px] font-bold text-slate-800 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"
            >
            ...
            <div
              className="flex h-[52px] min-w-0 items-center justify-between rounded-2xl bg-slate-100 p-1.5"
              aria-label="Фільтр кварталу"
            >
              {(["ALL", ...quarters] as QuarterFilter[]).map((item) => (
                <button
                  type="button"
                  key={item}
                  onClick={() => setQuarterFilter(item)}
                  className={`h-10 whitespace-nowrap rounded-xl px-2 text-[13px] font-extrabold transition sm:px-4 sm:text-[16px] ${quarterFilter === item ? "bg-white text-slate-900 shadow-[0_2px_6px_rgba(15,23,42,0.13)]" : "text-slate-500 hover:text-slate-800"}`}
                >
            ...
            {canEdit && !isSelectingForExtension && (
              <button
                ...
                className="flex h-[52px] min-w-0 items-center justify-center gap-2 whitespace-nowrap rounded-2xl border border-indigo-200 bg-white px-3 text-[14px] font-extrabold text-indigo-600 shadow-[0_2px_5px_rgba(15,23,42,0.10)] transition hover:border-indigo-400 hover:bg-indigo-50 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400 disabled:shadow-none lg:px-4 lg:text-[16px]"
                title={
                  eligibleIds.size === 0
            ...
            {canEdit && isSelectingForExtension && (
              <div
                className="flex h-[52px] min-w-0 items-center justify-between gap-2 rounded-2xl border border-indigo-100 bg-indigo-50 p-1.5 pl-3 sm:col-span-2 2xl:col-span-1"
                aria-label="Підтвердження продовження"
              >
                <span className="min-w-0 whitespace-nowrap text-[14px] font-extrabold text-indigo-800 lg:text-[16px]">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <button
                    ...
                    className="flex h-10 items-center rounded-xl bg-indigo-600 px-3 text-[13px] font-extrabold text-white shadow-[0_2px_5px_rgba(79,70,229,0.28)] transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-indigo-300 disabled:shadow-none lg:px-4 lg:text-[14px]"
                  >
                    Підтвердити
                  </button>
                  <button
                    type="button"
                    onClick={cancelExtensionSelection}
                    className="flex h-10 items-center rounded-xl border border-slate-300 bg-white px-3 text-[13px] font-extrabold text-slate-700 shadow-[0_2px_5px_rgba(15,23,42,0.10)] transition hover:bg-slate-50 lg:px-4 lg:text-[14px]"
                  >
                    Скасувати
                  </button>
                </div>
            ...
            {canEdit && !isSelectingForExtension && (
              <button
                ...
                className="flex h-[52px] min-w-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-2xl bg-indigo-600 px-2 text-[14px] font-extrabold text-white shadow-[0_3px_7px_rgba(79,70,229,0.28)] transition hover:bg-indigo-700 lg:px-3"
              >
      ...
      <section>
        <div className="flex items-end gap-5 border-b border-slate-200">
          <button
            ...
            className={`flex h-[68px] items-center gap-3 border-b-[3px] px-5 text-[20px] font-extrabold transition ${activeTab === "PROJECTS" ? "border-indigo-600 bg-indigo-50/70 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-800"}`}
          >
            <FolderOpen
              size={24}
              className={
          ...
          <button
            ...
            className={`flex h-[68px] items-center gap-3 border-b-[3px] px-5 text-[20px] font-extrabold transition ${activeTab === "TASKS" ? "border-indigo-600 bg-indigo-50/70 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-800"}`}
          >
            <ClipboardList
              size={23}
              className={
                activeTab === "TASKS" ? "text-indigo-500" : "text-slate-400"
              }
            />{" "}
      ...
      {notice && (
        <div
          role="status"
          className={`flex items-center justify-between rounded-2xl border px-4 py-3 text-sm font-bold ${notice.type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-800"}`}
        >
      ...
      {archive && (
        <section
          aria-label="Архівний період"
          className="flex flex-col items-start justify-between gap-4 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-amber-800 shadow-sm sm:flex-row sm:items-center sm:px-8"
        >
          <div className="flex items-center gap-4">
            <FolderOpen
              size={32}
              className="shrink-0 fill-amber-300 text-amber-500"
              aria-hidden="true"
            />
            <div>
              <h2 className="text-[22px] font-extrabold leading-7 text-amber-900">
                Архівний період ({selectedYear})
              </h2>
              <p className="text-[18px] font-semibold leading-6 text-amber-700">
          ...
          <button
            type="button"
            onClick={() => changeYear(currentPeriod.year)}
            className="inline-flex h-14 items-center gap-3 whitespace-nowrap rounded-2xl border border-amber-400 bg-white px-7 text-[18px] font-extrabold text-amber-700 shadow-sm transition hover:bg-amber-50 focus:outline-none focus:ring-4 focus:ring-amber-200"
          >
      ...
      <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-3 border-b border-slate-200 bg-slate-50/70 p-4 md:grid-cols-2 xl:grid-cols-[1.3fr_1.3fr_1fr_0.8fr] xl:p-5">
          <input
            ...
            className="h-14 rounded-2xl border border-slate-300 bg-white px-5 text-[17px] text-slate-800 outline-none placeholder:text-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"
          />
          <input
            value={goalSearch}
          ...
          <select
            ...
            className="h-14 rounded-2xl border border-slate-300 bg-white px-5 text-[17px] text-slate-800 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"
          >
                ...
                {isSelectingForExtension && (
                  <th className="w-16 px-4 py-4 text-center">
                    <input
                      ...
                      disabled={selectableMasterIds.length === 0}
                      onChange={toggleSelectAll}
                      className="h-5 w-5 cursor-pointer rounded-md border-slate-300 accent-indigo-600 disabled:cursor-not-allowed"
                    />
                  </th>
                )}
                <th className="px-4 py-4 text-left">
                  {activeTab === "PROJECTS" ? "Назва проєкту" : "Назва задачі"}
                </th>
                <th className="px-4 py-4 text-left">Стратегічна задача</th>
                {visibleQuarters.map((quarter) => (
                  <th key={quarter} className="w-[4.5rem] px-0 py-4">
                    <span className="block w-full text-center">{quarter}</span>
                  </th>
                ))}
                <th aria-label="Дії" className="w-24 py-4 pl-2 pr-3" />
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {masters.map((master) => {
                ...
                const manager = managers.find(
                  (item) => item.id === master.manager_id,
                );
                const priority = priorities.find(
                  (item) => item.id === master.priority,
                );
                ...
                return (
                  <React.Fragment key={master.id}>
                    <tr
                      className={`transition hover:bg-slate-50 ${isSelected ? "bg-indigo-50/50" : ""}`}
                    >
                      {isSelectingForExtension && (
                        <td className="px-4 py-4 text-center">
                          {isEligible ? (
                            <input
                              ...
                              checked={isSelected}
                              onChange={() => toggleSelected(master.id)}
                              className="h-5 w-5 cursor-pointer rounded-md border-slate-300 accent-indigo-600"
                            />
                          ) : (
                            <span
                              className="whitespace-nowrap rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700"
                              title={`Snapshot ${targetYear} вже існує`}
                            >
                              Продовжено
                            </span>
                          )}
                      ...
                      <td className="px-4 py-4">
                        <button
                          ...
                          onClick={() =>
                            setExpandedId(
                              expandedId === master.id ? null : master.id,
                            )
                          }
                          className="text-left text-[17px] font-extrabold leading-6 text-slate-900 hover:text-indigo-700"
                        >
                          {master.name}
                        </button>
                        <div className="mt-1 text-xs font-medium text-slate-400">
                          {master.id} · snapshot {selectedYear}
                        </div>
                      </td>
                      <td className="max-w-xs px-4 py-4">
                        <div
                          className="line-clamp-3 whitespace-pre-line text-[15px] leading-6 text-slate-600"
                      ...
                      {visibleQuarters.map((quarter) => {
                        ...
                        const isLockedQuarter = isPastQuarter(quarter);
                        const disabled = !canEdit || isLockedQuarter;
                        return (
                          <td
                            key={quarter}
                            className="w-[4.5rem] px-2 py-4 text-center"
                          >
                            <button
                              type="button"
                              disabled={disabled}
                              onClick={() => toggleQuarter(master, quarter)}
                              title={
                                isLockedQuarter
                            ...
                            {cards.length
                              ? [...cards]
                                  ...
                                  .map((card) => {
                                    ...
                                    const cardManager = managers.find(
                                      (item) => item.id === card.manager_id,
                                    );
                                    const cardPriority = priorities.find(
                                      (item) => item.id === card.priority,
                                    );
                                    const involved =
                                      card.cross_functional_dept_ids.map(
                                        (id) =>
                                          departments.find(
                                            (department) =>
                                              department.id === id,
                                    ...
                                    return (
                                      <button
                                        ...
                                        className="min-w-[292px] rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left shadow-sm transition hover:border-indigo-300 hover:shadow-md"
                                      ...
                                      >
                                        <div className="flex items-center justify-between gap-3">
                                          ...
                                          <span
                                            className="inline-flex min-w-24 justify-center rounded-full border px-2.5 py-1 text-[11px] font-bold"
                                        ...
                                        <div className="mt-3 space-y-1.5 text-xs leading-5 text-slate-600">
                                          ...
                                          <p className="flex items-center gap-2">
                                            ...
                                            <span
                                              className="inline-flex w-24 justify-center rounded-full border px-2 py-0.5 text-[10px] font-bold"
                                        ...
                                        <div
                                          ...
                                          role="progressbar"
                                          aria-label={`Прогрес scope ${card.quarter} ${card.year}`}
                                          aria-valuemin={0}
                                          aria-valuemax={100}
                              ...
                              : (() => {
                                  const stage = getYearSnapshot(
                                    master,
                                    selectedYear,
                                  )?.preparationStage;
                                  const stageManager = managers.find(
                                    (item) => item.id === stage?.manager_id,
                                  );
                                  const stagePriority = priorities.find(
                                    (item) => item.id === stage?.priority,
                                  );
                                  const involved = (
                                  ...
                                  return (
                                    <button
                                      type="button"
                                      onClick={() => setPreparationItem(master)}
                                      className="min-w-[292px] rounded-2xl border border-dashed border-indigo-200 bg-white px-4 py-3 text-left shadow-sm transition hover:border-indigo-400 hover:bg-indigo-50"
                                    ...
                                    >
                                      <div className="flex items-center justify-between gap-3">
                                        ...
                                        <span className="inline-flex min-w-24 justify-center rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-bold text-slate-600">
                                          Без статусу
                                        </span>
                                      </div>
                                      <div className="mt-3 space-y-1.5 text-xs leading-5 text-slate-600">
      ...
      {masterToDelete && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          ...
          >
            <div className="flex gap-3 border-b border-rose-200 bg-rose-50 p-5">
              <AlertTriangle className="shrink-0 text-rose-600" />
              <div>
                <h3
                  id="delete-backlog-title"
            ...
            <div className="p-5 text-sm leading-6 text-slate-700">
              ...
              <strong>«{masterToDelete.name}»</strong>? Якщо існують квартальні
              картки, система не дозволить видалення.
            </div>
            <div className="flex justify-end gap-3 border-t border-slate-100 bg-slate-50 p-4">
              <button
                type="button"
                onClick={() => setMasterToDelete(null)}
                className="rounded-xl px-4 py-2 font-bold text-slate-600 hover:bg-slate-200"
              >
                Скасувати
              </button>
              <button
