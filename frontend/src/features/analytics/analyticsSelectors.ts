import { AnalyticsFilters, AnalyticsKind, AnalyticsRecord, AnalyticsResponse } from './analyticsTypes';
...
export const recordsByIds = (data: AnalyticsResponse | undefined, ids?: string[]): AnalyticsRecord[] => {
  if (!data) return [];
...
export const statusCardIds = (data: AnalyticsResponse | undefined, statusId: string) =>
  data?.status_distribution?.find((status) => status.status_id === statusId)?.card_ids ?? [];
...
export const latestInitiativeRecords = (data: AnalyticsResponse | undefined): AnalyticsRecord[] => {
  ...
  (data.records ?? []).forEach((record) => {
    ...
    if (!latest.has(key) || latest.get(key)!.quarter < record.quarter) latest.set(key, record);
  });
...
export const analyticsKindLabels = (kind: AnalyticsKind) => {
  ...
  if (kind === 'OPERATIONAL_TASK') return {
...
export const analyticsStatusLabel = (status: 'GREEN' | 'YELLOW' | 'RED' | 'DEFAULT') =>
...
export const quarterlyDepartmentReserve = (
  ...
  const period = data.capacity_by_quarter.find((item) => item.quarter === quarter);
  return departments.map((department) => {
    const metric = period?.departments.find((item) => item.department_id === department.id);
    const load = metric?.load ?? 0;
    const limit = metric?.limit ?? 0;
    return { ...department, load, limit, reserve: limit - load, isOverCapacity: load > limit };
  });
};
