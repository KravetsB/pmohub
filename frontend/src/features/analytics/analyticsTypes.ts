...
export type StatusCounts = Record<'GREEN' | 'YELLOW' | 'RED' | 'DEFAULT', number>;
export interface CardStatusMetric { status_id: string; code: string; name: string; color: string; count: number; card_ids: string[] }
...
export interface AnalyticsResponse {
  ...
  summary: { cards: number; initiatives: number; total_weight: number; average_progress: number; average_duration: number; overloaded_departments: number };
  ...
  priority_status_breakdown: Array<{ priority_id: string | null; name: string; card_ids: string[]; status_counts: Record<string, number> }>;
  ...
  history: Array<{ year: number; cards: number; initiatives: number; status_distribution: CardStatusMetric[] }>;
...
export interface AnalyticsDrilldownResponse { records: AnalyticsRecord[]; page: number; page_size: number; total: number }
