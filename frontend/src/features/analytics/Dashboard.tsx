...
const statusColors: Record<keyof StatusCounts, string> = {
  DEFAULT: '#94a3b8', YELLOW: '#f59e0b', GREEN: '#10b981', RED: '#f43f5e',
};
const sizeColors = ['#c4b5fd', '#a78bfa', '#8b5cf6', '#7c3aed', '#6d28d9'];
const riskLabels: Record<string, string> = {
  NO_MANAGER: 'Без менеджера', NO_PRIORITY: 'Без пріоритету', NO_SCOPE: 'Без scope',
  NO_EXECUTOR: 'Без виконавця', INCOMPLETE_PREPARATION: 'Підготовчий етап заповнений не повністю',
};
  ...
  return <div className={styles.dashboard}>
    <section className={styles.toolbar}>
      <div className={styles.toolbarRow}>
        <div className={styles.filters}>
          <select value={filters.kind} onChange={(event) => update('kind', event.target.value as AnalyticsFilters['kind'])} className={`${styles.select} ${styles.kindSelect}`} aria-label="Тип записів"><option value="ALL">Проєкти + операційні задачі</option><option value="PROJECT">Тільки проєкти</option><option value="OPERATIONAL_TASK">Тільки операційні задачі</option></select>
          <select value={filters.year} onChange={(event) => update('year', Number(event.target.value))} className={styles.select} aria-label="Рік">{years.map((year) => <option key={year}>{year}</option>)}</select>
          {mode === 'quarterly' && <select value={filters.quarter} onChange={(event) => update('quarter', event.target.value as Quarter)} className={styles.select} aria-label="Квартал">{(['Q1', 'Q2', 'Q3', 'Q4'] as Quarter[]).map((quarter) => <option key={quarter}>{quarter}</option>)}</select>}
          <select value={filters.departmentId} onChange={(event) => update('departmentId', event.target.value)} className={styles.select} aria-label="Підрозділ"><option value="">Всі підрозділи</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
          <select value={filters.managerId} onChange={(event) => update('managerId', event.target.value)} className={styles.select} aria-label="Менеджер"><option value="">Всі менеджери</option>{managers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
        </div>
        <div className={styles.switch} aria-label="Період аналітики">{([['quarterly', 'Квартальний'], ['annual', 'Річний']] as const).map(([id, title]) => <button type="button" key={id} onClick={() => { setMode(id); setDrilldown(null); }} className={`${styles.switchButton} ${mode === id ? styles.switchActive : ''}`}>{title}</button>)}</div>
      </div>
      <p className={styles.filterNote}>{mode === 'quarterly' ? `Усі показники розраховано для ${filters.quarter} ${filters.year} року.` : 'Річні показники розраховано за всіма квартальними картками Q1–Q4.'}</p>
    </section>
    ...
    {data && <>
      <div className={styles.kpiGrid}>
        ...
        <Kpi title="Підрозділів понад ліміт" value={summary.overloaded_departments} accent={summary.overloaded_departments ? '#e11d48' : '#059669'} danger={summary.overloaded_departments > 0} onClick={() => setDrilldown({ type: 'departments', title: 'Підрозділи з перевищенням ліміту', departments: overloadedDepartments })} />
      </div>
      ...
      {mode === 'quarterly' ? <>
        ...
        <div className={styles.grid2}>
          ...
          <Chart title="Резерв завантаження" description={`Вільна вага до ліміту ${filters.quarter}.`} badge={summary.overloaded_departments ? `Перевантажено: ${summary.overloaded_departments}` : undefined} dangerBadge={summary.overloaded_departments > 0}><ReserveWidget data={data} mode={mode} quarter={filters.quarter} departments={activeDepartments} /></Chart>
        </div>
        <div className={styles.grid2}>
          <Chart title={`Динаміка обсягу ${kindLabels.genitive}: поточний і попередній квартал`}><QuarterComparisonChart data={data} /></Chart>
          <Chart title={`Топ завантажених менеджерів за вагою ${kindLabels.genitive}`}><ManagerCards data={data} onOpen={openRecords} compact /></Chart>
        </div>
      </> : <>
        ...
        <div className={styles.grid2}>
          ...
          <Chart title="Резерв завантаження" description="Вільна вага до річного ліміту." badge={summary.overloaded_departments ? `Перевантажено: ${summary.overloaded_departments}` : undefined} dangerBadge={summary.overloaded_departments > 0}><ReserveWidget data={data} mode={mode} quarter={filters.quarter} departments={activeDepartments} /></Chart>
        </div>
        <Chart title={`Топ завантажених менеджерів за вагою ${kindLabels.genitive}`}><ManagerCards data={data} onOpen={openRecords} /></Chart>
      </>}
...
const Kpi = ({ title, value, accent, progress, danger = false, onClick }: { title: string; value: string | number; accent: string; progress?: number; danger?: boolean; onClick?: () => void }) => {
  const Tag = onClick ? 'button' : 'div';
  return <Tag onClick={onClick} className={`${styles.kpi} ${danger ? styles.kpiDanger : ''}`} style={{ '--accent': accent } as CSSProperties}><div className={styles.kpiLabel}>{title}</div><div className={styles.kpiValue}>{value}</div>{progress !== undefined && <div className={styles.kpiTrack}><div className={styles.kpiTrackFill} style={{ width: `${Math.min(progress, 100)}%` }} /></div>}{onClick && <span className={styles.kpiHint}>Переглянути деталі →</span>}</Tag>;
};
const Chart = ({ title, description, badge, dangerBadge = false, children, className = '' }: React.PropsWithChildren<{ title: string; description?: string; badge?: string; dangerBadge?: boolean; className?: string }>) => <section className={`${styles.panel} ${className}`}><header className={styles.panelHeader}><div><h3 className={styles.panelTitle}>{title}</h3>{description && <p className={styles.panelDescription}>{description}</p>}</div>{badge && <span className={`${styles.badge} ${dangerBadge ? styles.badgeDanger : ''}`}>{badge}</span>}</header>{children}</section>;
const Empty = () => <div className={styles.empty}>Немає даних</div>;

const SizeChart = ({ data, onOpen }: { data: AnalyticsResponse; onOpen: (title: string, ids?: string[]) => void }) => {
  const height = Math.max(280, data.size_breakdown.length * 52);
  return <div className={styles.verticalChartScroll}><div style={{ height }}><ResponsiveContainer width="100%" height="100%"><BarChart data={data.size_breakdown} layout="vertical" margin={{ left: 8, right: 30 }}><CartesianGrid strokeDasharray="4 4" horizontal={false} stroke="#dbe5f0" /><XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: '#71829d' }} axisLine={false} tickLine={false} /><YAxis dataKey="name" type="category" width={62} tick={{ fontSize: 12, fill: '#475569', fontWeight: 750 }} axisLine={false} tickLine={false} /><Tooltip content={<SimpleTooltip valueLabel="Кількість карток" />} cursor={{ fill: '#f6f8fb' }} /><Bar dataKey="count" name="Кількість карток" radius={[0, 6, 6, 0]} onClick={(entry: any) => onOpen(`Розмір: ${entry.name ?? entry.payload?.name}`, entry.card_ids ?? entry.payload?.card_ids)}>{data.size_breakdown.map((item, index) => <Cell key={item.name} fill={sizeColors[index % sizeColors.length]} />)}</Bar></BarChart></ResponsiveContainer></div></div>;
};
...
const ScopeProgressChart = ({ data, statusColor }: { data: AnalyticsResponse; statusColor: (code: keyof StatusCounts) => string }) => {
  const rows = statusOrder.map((code) => ({ code, name: analyticsStatusLabel(code), value: data.scope_status_counts[code], color: statusColor(code) }));
...
const VolumeTooltip = ({ active, payload, label, currentYear }: any) => active && payload?.length ? <div className={styles.tooltip}><strong>{label}</strong><span style={{ color: '#94a3b8' }}>Карток {currentYear - 1}: {payload.find((item: any) => item.dataKey === 'previous')?.value ?? 0}</span><span style={{ color: '#6366f1' }}>Карток {currentYear}: {payload.find((item: any) => item.dataKey === 'current')?.value ?? 0}</span></div> : null;

const ManagerCards = ({ data, onOpen, compact = false }: { data: AnalyticsResponse; onOpen: (title: string, ids?: string[]) => void; compact?: boolean }) => data.manager_loads.length ? <div className={`${styles.managerGrid} ${compact ? styles.managerGridCompact : ''}`}>{data.manager_loads.slice(0, 5).map((manager, index) => <button type="button" key={manager.manager_id} className={styles.managerCard} onClick={() => onOpen(`Менеджер: ${manager.name}`, manager.card_ids)}><span className={`${styles.managerRank} ${index === 0 ? styles.managerRankTop : ''}`}>#{index + 1}</span><div className={styles.managerName} title={manager.name}>{manager.name}</div><div className={styles.managerLoad}>{manager.load} <span className={styles.managerUnit}>бал.</span></div></button>)}</div> : <Empty />;
...
const ReserveWidget = ({ data, mode, quarter, departments }: { data: AnalyticsResponse; mode: AnalyticsMode; quarter: Quarter; departments: Array<{ id: string; name: string }> }) => {
  ...
  const metrics = mode === 'annual' ? data.department_capacity.filter((item) => departments.some((department) => department.id === item.department_id)).map((item) => ({ id: item.department_id, name: item.name, load: item.load, limit: item.limit, reserve: item.reserve, isOverCapacity: item.is_over_capacity })) : quarterlyDepartmentReserve(data, quarter, departments);
  return <div className={styles.reserveGrid}>{metrics.map((department) => { const percent = department.limit ? Math.min(department.load / department.limit * 100, 100) : 0; return <article key={department.id} className={styles.reserveCard}><div className={styles.reserveTop}><span className={styles.departmentName} title={department.name}>{department.name}</span><span className={`${styles.reserveValue} ${reserveTone(department.load, department.limit)}`}>{department.reserve < 0 ? `Перевищення ${Math.abs(department.reserve)}` : `Резерв ${department.reserve}`}</span></div><div className={styles.progressTrack}><div className={`${styles.progressFill} ${department.isOverCapacity ? styles.progressDanger : ''}`} style={{ width: `${percent}%` }} /></div><div className={styles.loadMeta}><span>{department.load} з {department.limit} балів</span></div></article>; })}</div>;
};
