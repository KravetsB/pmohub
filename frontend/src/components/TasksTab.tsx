...
export const TasksTab = () => {
  ...
  if (filterPriority) {
    portfolioTasks = portfolioTasks.filter(t => t.priority === filterPriority);
  }
  if (searchQuery) {
    const q = searchQuery.toLowerCase();
    portfolioTasks = portfolioTasks.filter(t => t.name.toLowerCase().includes(q));
  }
  if (searchGoal) {
  ...
  const taskCustomFields = (customFields || []).filter(cf => cf.entityType === 'task' && cf.showInTable);
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm min-h-[400px]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Портфель Операційних задач</h2>
          <p className="text-slate-500 mt-1 text-sm">Всі задачі обраного періоду.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
          {portfolioTasks.map(t => (
            <div key={t.id} className="relative group h-fit">
              <TaskCard task={t} onClick={() => openEditModal(t)} hideColorPicker={!canEdit} />
            </div>
          ))}
        </div>
      ) : (
          ...
          <table className="min-w-max divide-y divide-slate-200 text-sm w-full table-fixed">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-widest w-80 min-w-[260px]">Назва задачі</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-widest w-48">Статус задачі</th>
                ...
                <tr key={t.id} className="transition-colors border-b border-slate-100/80" style={{ backgroundColor: colorWithAlpha(statusPresentation.color, .07) }}>
                  ...
                  <td className="px-4 py-3 min-w-0">
                    <div className="flex flex-wrap gap-1 max-w-full">
                      ...
                      {(!t.cross_functional_dept_ids || t.cross_functional_dept_ids.length === 0) && <span className="text-slate-400 text-xs">—</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3 min-w-0">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-600 min-w-0">
                    {t.notes ? <RichTextPreview value={t.notes} title={stripHtml(t.notes)} className="rich-text-card-preview transition-all" /> : '—'}
                  </td>
                  {taskCustomFields.map(cf => (
                    <td key={cf.id} className="px-4 py-3 text-xs text-slate-700 min-w-0">
                      <span className="block break-words line-clamp-2 hover:line-clamp-none transition-all" title={String(t.custom_fields?.[cf.id] ?? '')}>
                        {t.custom_fields?.[cf.id] !== undefined && t.custom_fields?.[cf.id] !== null && t.custom_fields?.[cf.id] !== '' ? String(t.custom_fields[cf.id]) : '—'}
                      </span>
                    </td>
                  ))}
        ...
        <TaskModal 
          ...
          onSave={(t, syncTargets) => { 
            if (editingTask) {
              ...
              if ((syncTargets ?? []).includes(editingTask.backlog_id ?? '')) backlogYears.push(editingTask.year);
              const cardIds = (syncTargets ?? []).filter(id => tasks.some(card => !card.is_backlog && card.id === id));
              const result = savePassport({ kind: 'task', source: { type: 'card', cardId: editingTask.id }, passportPatch: passportFrom(t), sourceCardPatch: { checklist: t.checklist, health_status: t.health_status }, targets: { backlogYears, cardIds } });
              if (!result.success) { alert(result.message); return; }
            } else {
              const master = { ...t, is_backlog: true, backlog_id: undefined, checklist: [], quarter: 'Q1' as const, yearSnapshots: { [String(t.year)]: { ...passportFrom(t), year: t.year, history: [] } } };
              const result = createBacklogWithCards('task', master, [t.quarter], t.checklist);
              if (!result.success) { alert(result.message); return; }
            }
