...
export const ProjectsTab = () => {
  ...
  const getRowBgClass = (status?: string) => {
    switch (status) {
      case "GREEN":
        return "bg-emerald-50/70 hover:bg-emerald-100/80 text-slate-800";
      case "YELLOW":
        return "bg-amber-50/70 hover:bg-amber-100/80 text-slate-800";
      case "RED":
        return "bg-rose-50/70 hover:bg-rose-100/80 text-slate-800";
      case "DEFAULT":
      default:
      ...
      {isArchive && (
        <div className="bg-amber-50 text-amber-800 px-5 py-3 rounded-xl font-medium text-sm mb-5 border border-amber-200/60 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <div>
              <strong className="block text-amber-900">
                ...
                {canEditArchive && (
                  <span className="ml-2 text-xs bg-amber-200 text-amber-900 font-bold px-2 py-0.5 rounded-md uppercase">
          ...
          <button
            ...
            className="bg-white/80 hover:bg-white text-amber-700 border border-amber-300 hover:border-amber-400 px-5 py-2 rounded-xl font-bold transition-all shadow-sm hover:shadow flex items-center gap-2 whitespace-nowrap group"
          >
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
          <div className="flex gap-2">
            <select
              ...
              className="border border-slate-300 rounded-lg px-2 py-1 text-sm bg-white focus:ring-indigo-500 outline-none truncate"
            >
          ...
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button
              ...
              className={`px-3 py-1 text-sm font-bold rounded-md ${viewMode === "grid" ? "bg-white shadow text-slate-800" : "text-slate-500 hover:text-slate-700"}`}
            >
              Картки
            </button>
            <button
              onClick={() => setViewMode("table")}
              className={`px-3 py-1 text-sm font-bold rounded-md ${viewMode === "table" ? "bg-white shadow text-slate-800" : "text-slate-500 hover:text-slate-700"}`}
            >
              Таблиця
            </button>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-x-auto w-full max-w-full min-w-0">
          <table className="min-w-max divide-y divide-slate-200 text-sm w-full table-fixed">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-widest w-36">
                  Менеджер
                </th>
                ...
                <th className="portfolio-header-center px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-widest w-40 min-w-[10rem]">
                  Пріоритет
                </th>
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-widest w-48">
                  Виконавці
                ...
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-widest w-72">
                  Скоуп зі статусами
                </th>
                <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-widest w-52">
                  Примітки
                  ...
                  >
                    ...
                    <td className="px-4 py-3 min-w-0">
                      <div className="flex flex-col items-start gap-2">
                        <span
                          className="px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider border whitespace-nowrap shadow-2xs"
                        ...
                        {canEdit && (
                          <div
                            className="flex items-center gap-1.5 rounded-lg border border-slate-200/80 bg-white/90 p-1 backdrop-blur shadow-2xs"
                            ...
                            {initiativeStatuses
                              ...
                              .map((item) => (
                                <button
                                  ...
                                  className={`w-3.5 h-3.5 rounded-full border transition-all ${status === item.id ? "ring-2 ring-offset-1 scale-110" : "hover:scale-125"}`}
                                  style={{
                    ...
                    <td className="px-4 py-3 text-xs text-slate-600 min-w-0">
                      <span
                        className="block break-words line-clamp-3 hover:line-clamp-none transition-all"
                        title={goalName}
                      >
                        {goalName}
                      </span>
                      ...
                      <div className="flex flex-wrap gap-1 max-w-full">
                        {Array.from(
                          ...
                          return (
                            <span
                              key={id}
                              className="text-[10px] px-1.5 py-0.5 bg-indigo-50 text-indigo-700 rounded border border-indigo-100 font-medium break-words max-w-full"
                              title={d.name}
                            >
                              {d.name}
                            </span>
                      ...
                      <div className="flex flex-wrap gap-1 max-w-full">
                        {(p.cross_functional_dept_ids || []).map((id) => {
                          ...
                          return (
                            <span
                              key={id}
                              className="text-[10px] px-1.5 py-0.5 bg-slate-100 text-slate-600 rounded border border-slate-200 break-words max-w-full"
                              title={d.name}
                            >
                              {d.name}
                            </span>
                        ...
                        <ul className="flex flex-col gap-1 max-h-36 overflow-y-auto pr-1">
                          {p.checklist.map((c) => (
                            ...
                            >
                              ...
                              />
                              <span
                                className={`leading-snug break-words ${c.color === "GREEN" ? "text-emerald-700" : c.color === "YELLOW" ? "text-amber-700" : c.color === "RED" ? "text-rose-700" : "text-slate-600"}`}
                                title={c.text}
                              >
                                {c.text}
                              </span>
