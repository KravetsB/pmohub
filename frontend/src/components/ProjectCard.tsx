...
export const ProjectCard: React.FC<{ project: Project; onClick?: () => void; hideColorPicker?: boolean; isBacklogView?: boolean }> = ({ project, onClick, hideColorPicker, isBacklogView }) => {
  ...
  const cardFields = (customFields || []).filter(cf => cf.entityType === 'project' && cf.showInCards);
  const effectiveStatus = project.is_backlog ? 'DEFAULT' : project.health_status;
  ...
  const getSizeName = (checklist: any[]) => project.sizeSnapshot?.name ?? getInitiativeSize(getInitiativeWeight(checklist, taskWeights || []), initiativeSizes || []);
  ...
  return (
    ...
    >
      ...
      {!hideColorPicker && !project.is_backlog && (
      <div className="absolute top-2 right-2 flex flex-col opacity-0 group-hover:opacity-100 transition-opacity z-10 bg-white/95 backdrop-blur shadow-lg border border-slate-200 rounded-lg py-1.5 px-2.5">
        ...
        <div className="flex items-center gap-2">{initiativeStatuses.filter(item => item.is_active || item.id === effectiveStatus).map(item => <button key={item.id} onClick={(e) => { e.stopPropagation(); updateProject(project.id, { health_status: item.id }); }} className="w-3.5 h-3.5 rounded-full border hover:scale-125 transition-transform" style={{ backgroundColor: item.color, borderColor: colorWithAlpha(item.color, .8) }} title={item.name} />)}</div>
      </div>
      )}

      <div className="ml-2 flex justify-between items-start mb-3 gap-2">
        <div className="min-w-0 flex-1">
          <h3 className="font-bold text-slate-800 text-base leading-tight pr-6 line-clamp-2 break-words" title={project.name}>{project.name}</h3>
          <p className="text-xs text-slate-500 mt-1 uppercase tracking-wider">{project.id}</p>
        </div>
      </div>
      ...
      <div className="ml-2 grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4 mb-3 flex-1">
        ...
        <div className="flex justify-between items-center text-xs sm:text-sm">
          ...
          <span className="rounded border px-2 py-0.5 text-xs font-bold" style={getPriorityBadgeStyle(project.priority, priorities)}>
            {getPriorityLabel(project.priority)}
          </span>
        </div>
        <div className="flex justify-between items-center text-xs sm:text-sm">
          <span className="text-slate-500">Стратегічна задача:</span>
          <span className={`font-bold text-right pl-2 ${project.strategic_goal?.trim() ? 'text-emerald-600' : 'text-slate-500'}`} title={project.strategic_goal?.trim() ? 'Стратегічна задача заповнена' : 'Стратегічна задача відсутня'}>
            {project.strategic_goal?.trim() ? '✓' : '—'}
          </span>
        </div>
        <div className="flex justify-between items-center text-xs sm:text-sm">
          <span className="text-slate-500">Розмір/Період:</span>
          <span className="font-bold text-slate-800 pl-2">
            {getSizeName(project.checklist || [])} • {project.year} {project.quarter}
          </span>
        </div>
      </div>
        ...
        <div className="ml-2 mb-4 flex-col gap-3">
          ...
          {project.notes && (
            <div className="bg-slate-50 border border-slate-100 rounded-lg p-2.5">
              ...
              <RichTextPreview value={project.notes} title={stripHtml(project.notes)} className="rich-text-card-preview text-xs italic text-slate-600 sm:text-sm" />
            </div>
          )}
        </div>
      )}
      ...
      {cardFields.length > 0 && (
        <div className="ml-2 mb-4 grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4 border-t border-slate-100 pt-3">
          {cardFields.map(cf => {
            const val = project.custom_fields?.[cf.id];
