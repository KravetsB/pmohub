...
export class InitiativesService {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (initial) {
        await this.assertReferences(
          ...
          initial.manager_id,
          initial.priority_id,
          [...initial.department_ids, ...initial.scope.flatMap((item) => item.executor_department_ids)],
        );
      }
      if (initial) {
        this.assertOpen(dto.year, initial.quarter);
        if (initial.status_id) await this.assertCardStatus(tx, initial.status_id);
            ...
            create: {
              ...
              preparationStage: {
                create: {
                  ...
                  departments: {
                    createMany: { data: unique(dto.preparation.department_ids).map((departmentId) => ({ departmentId })) },
                  },
                },
              },
            },
      ...
      if (initial) {
        const weights = await this.loadWeights(tx, initial.scope.map((item) => item.weight_definition_id));
        const card = await this.createEmptyCard(tx, yearId, qn(initial.quarter), {
        ...
        if (initial.status_id) await tx.quarterCard.update({ where: { id: card.id }, data: { statusId: initial.status_id } });
        for (const item of initial.scope) {
          ...
          await tx.scopeItem.create({
            data: {
              ...
              executors: { createMany: { data: unique(item.executor_department_ids).map((departmentId) => ({ departmentId })) } },
            },
          });
        }
        const executorIds = initial.scope.flatMap((item) => item.executor_department_ids);
      ...
      await this.audit(tx, 'InitiativeYear', yearId, 'INITIATIVE_CREATED', 'Створено запис у беклозі', actor);
      return { initiative_id: initiative.id, initiative_revision: initiative.revision, year_id: yearId, year_revision: 1, card_id: cardId };
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    return ok('Ініціативу створено', result);
  }

  async updateInitiative(id: string, dto: UpdateInitiativeDto, actor: AuthUser) {
    await this.assertCanEdit(actor);
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      await this.audit(tx, 'Initiative', id, 'INITIATIVE_RENAMED', 'Змінено глобальну назву ініціативи', actor);
  ...
  async updateYear(id: string, dto: UpdateInitiativeYearDto, actor: AuthUser) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const changed = await tx.initiativeYear.updateMany({
        where: { id, revision: dto.revision },
        data: { strategicGoal: dto.strategic_goal?.trim() || null, revision: { increment: 1 } },
      });
      if (!changed.count) await this.throwConflict(tx, 'InitiativeYear', id);
      await this.audit(tx, 'InitiativeYear', id, 'YEAR_GOAL_UPDATED', 'Змінено стратегічну задачу року', actor);
      return { id, revision: dto.revision + 1 };
    });
  ...
  async updateBacklog(id: string, dto: UpdateBacklogDto, actor: AuthUser) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (!rootChanged.count) await this.throwConflict(tx, 'Initiative', year.initiativeId);
      const yearChanged = await tx.initiativeYear.updateMany({
        where: { id, revision: dto.year_revision },
        data: { strategicGoal: dto.strategic_goal?.trim() || null, revision: { increment: 1 } },
      });
      if (!yearChanged.count) await this.throwConflict(tx, 'InitiativeYear', id);
      await this.audit(tx, 'InitiativeYear', id, 'BACKLOG_UPDATED', 'Оновлено назву та стратегічну задачу', actor);
      return {
  ...
  async updatePreparation(id: string, dto: UpdatePreparationDto, actor: AuthUser) {
    await this.assertCanEdit(actor);
    const result = await this.prisma.$transaction(async (tx) => {
      const year = await tx.initiativeYear.findUnique({ where: { id }, include: { preparationStage: true } });
      if (!year?.preparationStage) throw this.notFound('Підготовчий етап');
      if (this.yearLocked(year.year)) throw this.archived();
      await this.assertReferences(tx, dto.manager_id, dto.priority_id, dto.department_ids);
      const changed = await tx.preparationStage.updateMany({
        where: { initiativeYearId: id, revision: dto.revision },
        data: { managerId: dto.manager_id ?? null, priorityId: dto.priority_id ?? null, revision: { increment: 1 } },
      });
      ...
      await this.audit(tx, 'PreparationStage', id, 'PREPARATION_UPDATED', 'Оновлено підготовчий етап', actor);
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const year = await tx.initiativeYear.findUnique({
        ...
        include: {
          ...
          quarterCards: { where: { quarter: { lt: qn(dto.quarter) } }, include: { departments: true, scopeItems: { include: { executors: true } } }, orderBy: { quarter: 'desc' } },
        },
      });
      if (!year) throw this.notFound('Рік ініціативи');
      this.assertOpen(year.year, dto.quarter);
      const occupied = await tx.quarterCard.findUnique({ where: { initiativeYearId_quarter: { initiativeYearId: yearId, quarter: qn(dto.quarter) } } });
      if (occupied) throw this.targetOccupied();
      const previous = year.quarterCards[0];
      const defaults: Defaults = previous
        ? { managerId: previous.managerId, priorityId: previous.priorityId, departmentIds: this.effectiveDepartmentIds(previous) }
        : {
            managerId: year.preparationStage?.managerId ?? null,
            priorityId: year.preparationStage?.priorityId ?? null,
            departmentIds: year.preparationStage?.departments.map((item) => item.departmentId) ?? [],
          };
      const card = await this.createEmptyCard(tx, yearId, qn(dto.quarter), defaults);
      await this.audit(tx, 'QuarterCard', card.id, 'CARD_CREATED', 'Створено квартальну картку', actor, year.year, dto.quarter);
      return { card_id: card.id, card_revision: card.revision, year_id: yearId };
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
  ...
  async updateCard(id: string, dto: UpdateCardDto, actor: AuthUser) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const current = await tx.quarterCard.findUnique({
        where: { id },
        include: { initiativeYear: { include: { initiative: true } }, scopeItems: { include: { executors: true } } },
      });
      if (!current) throw this.notFound('Картку');
      if (isPeriodLocked(current.initiativeYear.year, qs(current.quarter))) throw this.archived();
      await this.assertReferences(tx, dto.manager_id, dto.priority_id, [...dto.department_ids, ...dto.scope.flatMap((item) => item.executor_department_ids)]);
      await this.assertCardStatus(tx, dto.status_id);
      const weights = await this.loadWeights(tx, dto.scope.map((item) => item.weight_definition_id));
      ...
      const incomingIds = new Set(dto.scope.flatMap((item) => item.id ? [item.id] : []));
      const removedIds = current.scopeItems.filter((item) => !incomingIds.has(item.id)).map((item) => item.id);
      if (removedIds.length) {
        await tx.scopeItem.updateMany({ where: { copiedFromItemId: { in: removedIds } }, data: { copiedFromItemId: null } });
      }
      await tx.scopeItem.deleteMany({ where: { quarterCardId: id, id: { notIn: [...incomingIds] } } });
      for (const item of dto.scope) {
        ...
        if (item.id) {
          ...
          const currentExecutorIds = new Set(
            current.scopeItems.find((existing) => existing.id === item.id)?.executors.map((link) => link.departmentId) ?? [],
          );
          await this.syncScopeExecutors(tx, item.id, nextExecutorIds, currentExecutorIds);
        } else {
          await tx.scopeItem.create({
            data: {
              quarterCardId: id,
      ...
      const executorIds = dto.scope.flatMap((item) => item.executor_department_ids);
  ...
  async extendYears(dto: ExtendYearsDto, actor: AuthUser) {
    await this.assertCanEdit(actor);
    const result = await this.prisma.$transaction(async (tx) => {
      const created: Array<{ source_year_id: string; target_year_id: string; revision: number }> = [];
      for (const sourceDto of dto.source_years) {
        const source = await tx.initiativeYear.findUnique({
          where: { id: sourceDto.id },
          include: {
            preparationStage: { include: { departments: true } },
            quarterCards: { include: { departments: true, scopeItems: { include: { executors: true } } }, orderBy: { quarter: 'desc' } },
          },
        });
        if (!source) throw this.notFound('Вихідний рік');
        await this.assertAggregateRevision(tx, 'InitiativeYear', source.id, sourceDto.revision);
        if (dto.target_year !== source.year + 1) {
          throw new AppError('INVALID_EXTENSION_YEAR', 'Ініціативу можна продовжити лише на наступний рік.', HttpStatus.BAD_REQUEST, { source_year: source.year, target_year: dto.target_year });
        }
        const existing = await tx.initiativeYear.findUnique({ where: { initiativeId_year: { initiativeId: source.initiativeId, year: dto.target_year } } });
        if (existing) throw new AppError('YEAR_ALREADY_EXISTS', 'Ініціативу вже продовжено на обраний рік.', HttpStatus.CONFLICT, { initiative_year_id: existing.id });
        const latest = source.quarterCards[0];
        const defaults: Defaults = latest
          ? { managerId: latest.managerId, priorityId: latest.priorityId, departmentIds: this.effectiveDepartmentIds(latest) }
          : {
              managerId: source.preparationStage?.managerId ?? null,
              priorityId: source.preparationStage?.priorityId ?? null,
              departmentIds: source.preparationStage?.departments.map((item) => item.departmentId) ?? [],
            };
        const target = await this.createYear(tx, source.initiativeId, dto.target_year, defaults);
        await this.audit(tx, 'InitiativeYear', target.id, 'YEAR_EXTENDED', 'Ініціативу продовжено на наступний рік', actor, source.year, undefined, dto.target_year);
        created.push({ source_year_id: source.id, target_year_id: target.id, revision: target.revision });
      }
  ...
  async moveCard(id: string, dto: PeriodCommandDto, actor: AuthUser) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const source = await tx.quarterCard.findUnique({
        where: { id },
        include: { initiativeYear: { include: { initiative: true } }, departments: true, scopeItems: { include: { executors: true } } },
      });
      if (!source) throw this.notFound('Картку');
      if (isPeriodLocked(source.initiativeYear.year, qs(source.quarter))) throw this.archived();
      this.assertOpen(dto.to_year, dto.to_quarter);
      if (source.initiativeYear.year === dto.to_year && source.quarter === qn(dto.to_quarter)) {
        throw new AppError('SAME_TARGET_PERIOD', 'Оберіть інший квартал.', HttpStatus.BAD_REQUEST);
      }
      const targetYear = await this.ensureYearFromCard(tx, source, dto.to_year);
      const occupied = await tx.quarterCard.findUnique({ where: { initiativeYearId_quarter: { initiativeYearId: targetYear.id, quarter: qn(dto.to_quarter) } } });
  ...
  async continueCard(id: string, dto: PeriodCommandDto, actor: AuthUser) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      this.assertOpen(dto.to_year, dto.to_quarter);
      if (dto.to_year * 10 + qn(dto.to_quarter) <= source.initiativeYear.year * 10 + source.quarter) {
        throw new AppError('INVALID_CONTINUATION_PERIOD', 'Продовження можливе лише у пізніший квартал.', HttpStatus.BAD_REQUEST);
      }
      const targetYear = await this.ensureYearFromCard(tx, source, dto.to_year);
      const occupied = await tx.quarterCard.findUnique({ where: { initiativeYearId_quarter: { initiativeYearId: targetYear.id, quarter: qn(dto.to_quarter) } } });
      if (occupied) throw this.targetOccupied();
      const card = await this.createEmptyCard(tx, targetYear.id, qn(dto.to_quarter), {
      ...
      await this.audit(tx, 'QuarterCard', card.id, 'CARD_CONTINUED', 'Ініціативу продовжено в інший квартал', actor, source.initiativeYear.year, qs(source.quarter), dto.to_year, dto.to_quarter);
      return { source_card_id: id, target_card_id: card.id, target_card_revision: card.revision, target_year_id: targetYear.id };
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    return ok('Ініціативу продовжено', result);
  }

  async moveScope(cardId: string, itemId: string, dto: PeriodCommandDto, actor: AuthUser) {
    return this.transferScope('MOVE', cardId, itemId, dto, actor);
  }

  async copyScope(cardId: string, itemId: string, dto: PeriodCommandDto, actor: AuthUser) {
  ...
  private async transferScope(mode: 'MOVE' | 'COPY', cardId: string, itemId: string, dto: PeriodCommandDto, actor: AuthUser) {
    await this.assertCanEdit(actor);
    const result = await this.prisma.$transaction(async (tx) => {
      const source = await tx.quarterCard.findUnique({
        where: { id: cardId },
      ...
      if (!source) throw this.notFound('Картку');
      await this.assertAggregateRevision(tx, 'QuarterCard', cardId, dto.revision);
      if (mode === 'MOVE' && isPeriodLocked(source.initiativeYear.year, qs(source.quarter))) throw this.archived();
      this.assertOpen(dto.to_year, dto.to_quarter);
      const item = source.scopeItems.find((candidate) => candidate.id === itemId);
      if (!item) throw this.notFound('Завдання скоупу');
      if (item.statusCode === 'GREEN') throw new AppError('COMPLETED_SCOPE_IMMUTABLE', 'Виконане завдання не можна переносити або копіювати.', HttpStatus.CONFLICT);
      const targetYear = await this.ensureYearFromCard(tx, source, dto.to_year);
      let target = await tx.quarterCard.findUnique({
        where: { initiativeYearId_quarter: { initiativeYearId: targetYear.id, quarter: qn(dto.to_quarter) } },
        include: { departments: true },
      });
      const targetExisted = Boolean(target);
      if (target?.id === source.id) throw new AppError('SAME_TARGET_PERIOD', 'Оберіть інший квартал.', HttpStatus.BAD_REQUEST);
      if (target && dto.target_revision !== undefined && target.revision !== dto.target_revision) {
        throw this.conflict(target.revision, 'QuarterCard', target.id);
      }
      if (!target) {
        target = await this.createEmptyCard(tx, targetYear.id, qn(dto.to_quarter), {
          managerId: source.managerId,
          priorityId: source.priorityId,
          departmentIds: unique([...this.effectiveDepartmentIds(source), ...item.executors.map((link) => link.departmentId)]),
        }, source.notes);
        if (source.customFieldValues.length) {
          await tx.customFieldValue.createMany({
            data: source.customFieldValues.map((value) => ({
      ...
      const duplicate = await tx.scopeItem.findUnique({ where: { quarterCardId_lineageId: { quarterCardId: target.id, lineageId: item.lineageId } } });
      if (duplicate) throw new AppError('SCOPE_LINEAGE_CONFLICT', 'Це завдання вже існує у цільовій картці.', HttpStatus.CONFLICT, { scope_item_id: duplicate.id, target_card_id: target.id });

      let createdScopeItemId: string | undefined;
      if (mode === 'MOVE') {
        const sourceChanged = await tx.quarterCard.updateMany({ where: { id: source.id, revision: dto.revision }, data: { revision: { increment: 1 } } });
        if (!sourceChanged.count) await this.throwConflict(tx, 'QuarterCard', source.id);
        const moved = await tx.scopeItem.updateMany({
          where: { id: item.id, quarterCardId: source.id, revision: item.revision },
          data: { quarterCardId: target.id, movedFromCardId: source.id, revision: { increment: 1 } },
        });
        ...
        const copied = await tx.scopeItem.create({
          data: {
            ...
            executors: { createMany: { data: item.executors.map((link) => ({ departmentId: link.departmentId })) } },
          },
        });
      ...
      if (targetExisted) {
        if (dto.target_revision === undefined) {
          throw new AppError('TARGET_REVISION_REQUIRED', 'Оновіть цільову картку перед зміною її скоупу.', HttpStatus.CONFLICT, { target_card_id: target.id, actual_revision: target.revision });
        }
        const targetChanged = await tx.quarterCard.updateMany({ where: { id: target.id, revision: dto.target_revision }, data: { revision: { increment: 1 } } });
        if (!targetChanged.count) await this.throwConflict(tx, 'QuarterCard', target.id);
      }
    ...
    return ok(mode === 'MOVE' ? 'Завдання перенесено' : 'Завдання скопійовано', result);
  }
  ...
  private async assertAggregateRevision(
    ...
    const delegate = aggregateType === 'InitiativeYear' ? tx.initiativeYear : tx.quarterCard;
  ...
  private async createYear(tx: Tx, initiativeId: string, year: number, defaults: Defaults) {
      ...
      data: {
        ...
        preparationStage: {
          create: {
            ...
            departments: { createMany: { data: unique(defaults.departmentIds).map((departmentId) => ({ departmentId })) } },
          },
        },
      },
    });
  ...
  private async detachCardMetadata(tx: Tx, cardIds: string[]) {
    ...
    const scopeIds = (await tx.scopeItem.findMany({ where: { quarterCardId: { in: cardIds } }, select: { id: true } })).map((item) => item.id);
    if (scopeIds.length) await tx.scopeItem.updateMany({ where: { copiedFromItemId: { in: scopeIds } }, data: { copiedFromItemId: null } });
    await tx.scopeItem.updateMany({ where: { movedFromCardId: { in: cardIds } }, data: { movedFromCardId: null } });
  }

  private async ensureYearFromCard(tx: Tx, source: any, targetYear: number) {
    const existing = await tx.initiativeYear.findUnique({
      where: { initiativeId_year: { initiativeId: source.initiativeYear.initiativeId, year: targetYear } },
    });
  ...
  private async createEmptyCard(tx: Tx, yearId: string, quarter: number, defaults: Defaults, notes?: string | null) {
    ...
    return tx.quarterCard.create({
      data: {
        ...
        departments: {
          createMany: { data: unique(defaults.departmentIds).map((departmentId) => ({ departmentId })) },
        },
      },
      include: { departments: true },
    });
  ...
  private async recalculateCard(tx: Tx, cardId: string) {
    const scope = await tx.scopeItem.findMany({ where: { quarterCardId: cardId } });
    const total = scope.reduce((sum, item) => sum + item.weightSnapshotValue.toNumber(), 0);
    const sizes = await tx.initiativeSize.findMany({ where: { isActive: true }, orderBy: { minScore: 'asc' } });
    const definition = sizes.find((size) => total >= size.minScore.toNumber() && total <= size.maxScore.toNumber());
  ...
  private async replacePreparationDepartments(tx: Tx, yearId: string, departmentIds: string[]) {
    ...
    const existing = await tx.preparationStageDepartment.findMany({ where: { initiativeYearId: yearId, departmentId: { in: ids } }, select: { departmentId: true } });
    const existingIds = new Set(existing.map((item) => item.departmentId));
    await tx.preparationStageDepartment.createMany({ data: ids.filter((id) => !existingIds.has(id)).map((departmentId) => ({ initiativeYearId: yearId, departmentId })) });
  }

  private async syncScopeExecutors(tx: Tx, scopeItemId: string, nextExecutorIds: string[], currentExecutorIds: Set<string>) {
    await tx.scopeItemExecutor.deleteMany({ where: { scopeItemId, departmentId: { notIn: nextExecutorIds } } });
    const addedExecutorIds = nextExecutorIds.filter((departmentId) => !currentExecutorIds.has(departmentId));
    if (addedExecutorIds.length) {
      await tx.scopeItemExecutor.createMany({ data: addedExecutorIds.map((departmentId) => ({ scopeItemId, departmentId })) });
    }
  }

  private async replaceCardDepartments(tx: Tx, cardId: string, departmentIds: string[]) {
    const ids = unique(departmentIds);
    await tx.quarterCardDepartment.deleteMany({ where: { quarterCardId: cardId, departmentId: { notIn: ids } } });
    const existing = await tx.quarterCardDepartment.findMany({ where: { quarterCardId: cardId, departmentId: { in: ids } }, select: { departmentId: true } });
    const existingIds = new Set(existing.map((item) => item.departmentId));
    await tx.quarterCardDepartment.createMany({ data: ids.filter((id) => !existingIds.has(id)).map((departmentId) => ({ quarterCardId: cardId, departmentId })) });
  }

  private async replaceCustomFields(tx: Tx, cardId: string, kind: string, values: Record<string, unknown>) {
    const presentValues = Object.fromEntries(Object.entries(values).filter(([, value]) => value !== '' && value !== null && value !== undefined));
    ...
    for (const definition of definitions) {
      ...
      if (definition.fieldType === 'SELECT' && !definition.options.some((option) => option.value === String(raw))) {
        throw new AppError('INVALID_CUSTOM_FIELD_OPTION', `Недопустиме значення поля «${definition.name}».`);
      }
      const data = this.customFieldValue(definition.fieldType, raw);
      await tx.customFieldValue.upsert({
        where: { quarterCardId_definitionId: { quarterCardId: cardId, definitionId: definition.id } },
  ...
  private customFieldValue(type: string, value: unknown) {
    const empty = { textValue: null, numberValue: null, booleanValue: null, dateValue: null, optionValue: null };
    switch (type) {
      case 'NUMBER': {
  ...
  private async loadWeights(tx: Tx, ids: string[]) {
    ...
    const weights = await tx.taskWeight.findMany({ where: { id: { in: uniqueIds }, isActive: true } });
    if (weights.length !== uniqueIds.length) throw new AppError('INVALID_WEIGHT', 'Оберіть активну вагу для кожного завдання.');
    return new Map(weights.map((weight) => [weight.id, weight]));
  }

  private assertScopePayload(existing: Array<{ id: string; revision: number }>, incoming: UpdateCardDto['scope']) {
  ...
  private assertNewScopePayload(incoming: InitialQuarterCardDto['scope']) {
    const lineages = incoming.flatMap((item) => item.lineage_id ? [item.lineage_id] : []);
    if (new Set(lineages).size !== lineages.length) throw new AppError('DUPLICATE_SCOPE_ITEM', 'Завдання скоупу дублюється у запиті.');
    for (const item of incoming) {
      if (!item.text.trim()) throw new AppError('VALIDATION_ERROR', 'Текст завдання не може бути порожнім.');
      if (!item.executor_department_ids.length) throw new AppError('VALIDATION_ERROR', 'Оберіть хоча б один підрозділ-виконавець.');
    }
  ...
  private effectiveDepartmentIds(card: { departments: Array<{ departmentId: string }>; scopeItems: Array<{ executors: Array<{ departmentId: string }> }> }) {
    const executors = new Set(card.scopeItems.flatMap((item) => item.executors.map((link) => link.departmentId)));
    return card.departments.map((link) => link.departmentId).filter((id) => !executors.has(id));
  }

  private async defaultCardStatus(tx: Tx) {
    const status = await tx.initiativeStatus.findUnique({ where: { code: 'DEFAULT' } });
    if (!status?.isActive) throw new AppError('SYSTEM_DICTIONARY_MISSING', 'Системний статус DEFAULT не налаштовано.', HttpStatus.INTERNAL_SERVER_ERROR);
    return status;
  }

  private async defaultWeight(tx: Tx) {
    const weight = await tx.taskWeight.findFirst({ where: { isDefault: true, isSystem: true, isActive: true } });
    if (!weight) throw new AppError('SYSTEM_DICTIONARY_MISSING', 'Системну вагу «Не визначено» не налаштовано.', HttpStatus.INTERNAL_SERVER_ERROR);
    return weight;
  }

  private async assertCardStatus(tx: Tx, id: string) {
    const status = await tx.initiativeStatus.findFirst({ where: { id, isActive: true } });
    if (!status) throw new AppError('INVALID_STATUS', 'Оберіть активний статус картки.');
  }

  private async assertReferences(tx: Tx, managerId: string | undefined, priorityId: string | undefined, departmentIds: string[]) {
    if (managerId && !await tx.manager.findFirst({ where: { id: managerId, isActive: true }, select: { id: true } })) {
      throw new AppError('INVALID_MANAGER', 'Оберіть активного менеджера.');
    }
    if (priorityId && !await tx.priority.findFirst({ where: { id: priorityId, isActive: true }, select: { id: true } })) {
      throw new AppError('INVALID_PRIORITY', 'Оберіть активний пріоритет.');
    }
    const ids = unique(departmentIds);
    if (ids.length && await tx.department.count({ where: { id: { in: ids }, isActive: true } }) !== ids.length) {
      throw new AppError('INVALID_DEPARTMENT', 'Один або кілька підрозділів недоступні.');
    }
  }
  ...
  private assertOpen(year: number, quarter: QuarterDto) {
    ...
    if (year * 10 + qn(quarter) < current.year * 10 + qn(current.quarter)) throw this.archived();
  }
  ...
  private targetOccupied() {
    return new AppError('TARGET_CARD_OCCUPIED', 'У цільовому кварталі вже існує картка цієї ініціативи.', HttpStatus.CONFLICT);
  }

  private archived() {
    return new AppError('ARCHIVED_PERIOD', 'Архівний період не можна змінювати.', HttpStatus.CONFLICT);
  }

  private notFound(label: string) {
    return new AppError('NOT_FOUND', `${label} не знайдено.`, HttpStatus.NOT_FOUND);
  }

  private conflict(actualRevision: number, aggregateType: string, aggregateId: string) {
    return new AppError('REVISION_CONFLICT', 'Запис уже змінено іншим користувачем. Оновіть дані.', HttpStatus.CONFLICT, {
  ...
  private async throwConflict(tx: Tx, aggregateType: 'Initiative' | 'InitiativeYear' | 'QuarterCard' | 'ScopeItem', id: string): Promise<never> {
    const delegate = aggregateType === 'Initiative'
      ? tx.initiative
      : aggregateType === 'InitiativeYear'
        ...
        : aggregateType === 'QuarterCard'
          ? tx.quarterCard
          : tx.scopeItem;
    const aggregate = await (delegate as any).findUnique({ where: { id }, select: { revision: true } });
  ...
  private async throwPreparationConflict(tx: Tx, id: string): Promise<never> {
    const stage = await tx.preparationStage.findUnique({ where: { initiativeYearId: id } });
  ...
  private async audit(
    ...
    await tx.auditEvent.create({
      data: { aggregateType, aggregateId, actionCode, message, actorUserId: actor.id, actorName: actor.name, sourceYear, sourceQuarter, targetYear, targetQuarter },
    });
  }
}
