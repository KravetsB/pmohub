...
export async function assertPeriodCapacity(tx: Prisma.TransactionClient, year: number, quarter: number) {
  const cards = await tx.quarterCard.findMany({ where: { quarter, initiativeYear: { year } }, include: { departments: true, scopeItems: { include: { executors: true } } } });
  ...
  const exceeded = departments.filter((department) => (loads.get(department.id) ?? 0) > department.capacityLimitPoints.toNumber()).map((department) => ({
