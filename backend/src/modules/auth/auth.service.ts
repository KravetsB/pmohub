...
export class AuthService {
  ...
  async login(email: string, password: string, userAgent?: string) {
    ...
    if (!user?.isActive) throw new AppError('INVALID_CREDENTIALS', 'Невірний email або пароль', HttpStatus.UNAUTHORIZED);
    if (!user.passwordHash) throw new AppError('PASSWORD_NOT_SET', 'Для цього облікового запису не задано пароль. Зверніться до адміністратора.', HttpStatus.CONFLICT);
    if (!(await argon2.verify(user.passwordHash, password))) throw new AppError('INVALID_CREDENTIALS', 'Невірний email або пароль', HttpStatus.UNAUTHORIZED);
  ...
  async logout(refreshToken?: string) {
    if (refreshToken) await this.prisma.refreshToken.updateMany({ where: { tokenHash: digest(refreshToken), revokedAt: null }, data: { revokedAt: new Date() } });
  ...
  private async issueSession(user: { id: string; name: string; email: string; role: string; departmentId: string | null; mustChangePassword: boolean }, userAgent?: string, replacedTokenId?: string) {
    ...
    const refreshToken = await this.jwt.signAsync({ sub: user.id, jti: refreshId, type: 'refresh' }, {
      secret: this.config.getOrThrow('JWT_REFRESH_SECRET'), expiresIn: `${refreshDays}d` as never,
    });
    await this.prisma.$transaction(async (tx) => {
      if (replacedTokenId) await tx.refreshToken.update({ where: { id: replacedTokenId }, data: { revokedAt: new Date(), replacedBy: refreshId } });
      await tx.refreshToken.create({ data: { id: refreshId, userId: user.id, tokenHash: digest(refreshToken), expiresAt, userAgent } });
    });
    return {
  ...
  private toPublicUser(user: { id: string; name: string; email: string; role: string; departmentId: string | null; mustChangePassword: boolean }) {
    return { id: user.id, name: user.name, email: user.email, role: user.role, department_id: user.departmentId ?? undefined, must_change_password: user.mustChangePassword };
  }
}
