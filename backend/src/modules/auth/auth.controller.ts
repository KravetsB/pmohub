...
export class AuthController {
  ...
  private assertAllowedOrigin(request: Request) {
    ...
    if (!origin) return;
    const allowed = (this.config.get<string>('FRONTEND_ORIGINS') ?? this.config.get<string>('FRONTEND_ORIGIN', 'http://localhost:3000')).split(',').map((item) => item.trim());
    if (!allowed.includes(origin)) throw new AppError('ORIGIN_FORBIDDEN', 'Недозволене джерело запиту', HttpStatus.FORBIDDEN);
  }
}
