...
describe('AnalyticsService aggregation contracts', () => {
  ...
  it('aggregates annual status, size, progress and priority matrix from every quarterly card', async () => {
    ...
    expect(result.status_counts).toMatchObject({ GREEN: 1, YELLOW: 1 });
    ...
    expect(result.priority_status_breakdown[0]).toMatchObject({ name: 'Priority', status_counts: { GREEN: 1, YELLOW: 1, RED: 0, DEFAULT: 0 } });
    expect(result.history[0].status_counts).toMatchObject({ GREEN: 1, YELLOW: 1 });
  });
});
