...
export class AnalyticsDrilldownDto extends AnalyticsFilterDto {
  ...
  @IsOptional() @IsIn(['Q1', 'Q2', 'Q3', 'Q4']) quarter?: 'Q1' | 'Q2' | 'Q3' | 'Q4';
...
export class AnalyticsDrilldownResponseDto {
  ...
  @ApiProperty({ type: AnalyticsDrilldownDataDto }) data!: AnalyticsDrilldownDataDto;
}
...
export class AnalyticsSummaryDataDto {
  ...
  @ApiProperty({ type: Object, additionalProperties: false }) summary!: {
    cards: number; initiatives: number; total_weight: number; average_progress: number;
    average_duration: number; overloaded_departments: number;
  };
  @ApiProperty({ type: [CardStatusMetricDto] }) status_distribution!: CardStatusMetricDto[];
  @ApiProperty({ type: StatusCountsDto }) scope_status_counts!: StatusCountsDto;
  @ApiProperty({ type: [Object] }) size_breakdown!: Array<{ name: string; count: number; card_ids: string[] }>;
  @ApiProperty({ type: [Object] }) priority_breakdown!: Array<{ priority_id: string | null; name: string; total_weight: number; card_ids: string[] }>;
  @ApiProperty({ type: [Object] }) priority_status_breakdown!: Array<{ priority_id: string | null; name: string; card_ids: string[]; status_counts: Record<string, number> }>;
  @ApiProperty({ type: [Object] }) department_capacity!: Array<{ department_id: string; name: string; load: number; limit: number; reserve: number; is_over_capacity: boolean }>;
  @ApiProperty({ type: [Object] }) capacity_by_quarter!: Array<{ quarter: string; departments: unknown[] }>;
  @ApiProperty({ type: [Object] }) manager_loads!: Array<{ manager_id: string; name: string; load: number; card_ids: string[] }>;
  @ApiProperty({ type: [Object] }) risks!: Array<{ id: string; name: string; risks: string[] }>;
  ...
  @ApiProperty({ type: Object }) preparation!: { total: number; ready: number; records: unknown[] };
}

export class AnalyticsSummaryResponseDto {
  @ApiProperty({ enum: [true] }) success!: true;
  @ApiProperty({ required: false }) message?: string;
  @ApiProperty({ type: AnalyticsSummaryDataDto }) data!: AnalyticsSummaryDataDto;
}
