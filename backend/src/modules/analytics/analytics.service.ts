...
import { AnalyticsDrilldownDto, AnalyticsFilterDto, QuarterlyAnalyticsFilterDto } from './analytics.dto';
...
type Card = Prisma.QuarterCardGetPayload<{
  include: {
    ...
    manager: true; priority: true; status: true; departments: true;
...
export class AnalyticsService {
  ...
  async annual(filter: AnalyticsFilterDto) {
    ...
    cards.forEach((card) => {
      ...
      if (!latestByInitiative.has(key) || latestByInitiative.get(key)!.quarter < card.quarter) latestByInitiative.set(key, card);
    });
    ...
    const history = [...historyByYear.entries()].sort(([a], [b]) => a - b).map(([year, yearCards]) => ({
      year, status_distribution: this.statusDistribution(yearCards),
    ...
    return {
      ...
      ...await this.aggregate(cards, true, filter.department_id, [...latestByInitiative.values()]),
      available_years: await this.availableYears(filter.kind),
  ...
  async drilldown(filter: AnalyticsDrilldownDto) {
    const quarter = filter.mode === 'quarterly' && filter.quarter ? Number(filter.quarter.slice(1)) : undefined;
    ...
    if (filter.status_id) cards = cards.filter((card) => card.statusId === filter.status_id);
    const total = cards.length;
    const start = (filter.page - 1) * filter.page_size;
    return { records: cards.slice(start, start + filter.page_size).map((card) => this.record(card)), page: filter.page, page_size: filter.page_size, total };
  }
  ...
  private async aggregate(cards: Card[], annual: boolean, departmentId?: string, riskCards: Card[] = cards) {
    ...
    const progress = annual
      ? (scopeItems.length ? Math.round(scopeItems.reduce((sum, item) => sum + this.scopeProgressValue(item.statusCode), 0) / scopeItems.length * 100) : 0)
      : (() => {
    ...
    const priorityStatusMap = new Map<string, { priority_id: string | null; name: string; card_ids: string[]; status_counts: Record<string, number> }>();
    cards.forEach((card) => {
      const key = card.priorityId ?? 'NONE';
      ...
      current.status_counts[card.statusId] = (current.status_counts[card.statusId] ?? 0) + 1;
      priorityStatusMap.set(key, current);
    });
    const risks = riskCards.map((card) => this.record(card)).filter((record) => record.risks.length).map((record) => ({ id: record.id, name: record.name, risks: record.risks }));
    ...
    return {
      ...
      status_distribution: this.statusDistribution(cards), scope_status_counts: scopeStatusCounts,
      ...
      manager_loads: [...managerMap.values()].sort((a, b) => b.load - a.load), risks,
    };
  }
  ...
  private record(card: Card) {
    ...
    if (!card.scopeItems.length) risks.push('NO_SCOPE'); if (card.scopeItems.some((item) => !item.executors.length)) risks.push('NO_EXECUTOR');
    return {
      ...
      priority_name: card.priority?.name ?? null, department_ids: card.departments.map((link) => link.departmentId),
      status_id: card.statusId, status_code: card.status.code, status_name: card.status.name, status_color: card.status.color,
  ...
  private progress(card: Card) { return card.scopeItems.length ? Math.round(card.scopeItems.reduce((sum, item) => sum + this.scopeProgressValue(item.statusCode), 0) / card.scopeItems.length * 100) : 0; }
  private scopeProgressValue(status: string) { return status === 'GREEN' ? 1 : status === 'YELLOW' ? 0.5 : 0; }
  private statusDistribution(cards: Card[]) {
    const result = new Map<string, { status_id: string; code: string; name: string; color: string; count: number; card_ids: string[] }>();
    cards.forEach((card) => {
      const current = result.get(card.statusId) ?? {
        status_id: card.statusId, code: card.status.code, name: card.status.name,
        color: card.status.color, count: 0, card_ids: [],
      };
      current.count += 1;
      current.card_ids.push(card.id);
      result.set(card.statusId, current);
    });
    return [...result.values()].sort((a, b) => b.count - a.count || a.name.localeCompare(b.name, 'uk'));
  }
  private emptyCounts(): Record<ScopeStatus, number> { return { GREEN: 0, YELLOW: 0, RED: 0, DEFAULT: 0 }; }
  private status(value: string): ScopeStatus { return SCOPE_STATUSES.includes(value as ScopeStatus) ? value as ScopeStatus : 'DEFAULT'; }

  private departmentLoads(cards: Card[]) {
    const loads = new Map<string, number>();
    cards.forEach((card) => {
      const executors = new Set<string>(card.scopeItems.flatMap((item) => item.executors.map((link) => link.departmentId)));
      card.scopeItems.forEach((item) => {
        const ids = [...new Set<string>(item.executors.map((link) => link.departmentId))];
      ...
      const involved: string[] = card.departments.map((link) => link.departmentId).filter((id) => !executors.has(id));
