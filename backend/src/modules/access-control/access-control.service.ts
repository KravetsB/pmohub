...
export class AccessControlService {
  ...
  async update(role: string, dto: UpdatePermissionDto, actor: AuthUser) {
    const actorPermission = await this.prisma.rolePermission.findUnique({ where: { role: actor.role } });
    if (actor.role !== 'SUPER_ADMIN' || !actorPermission?.canAccessAdmin || actorPermission.isReadOnly) {
